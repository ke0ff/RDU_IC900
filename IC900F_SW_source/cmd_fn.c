...
char* cmd_list[] = { CMD_1, CMD_2A, CMD_3, CMD_31, CMD_32, CMD_4, CMD_41, CMD_5, CMD_6, CMD_7, CMD_81, CMD_101, CMD_102, CMD_11, \
...
enum       cmd_enum{ ENUM_1, ENUM_2A, ENUM_3, ENUM_31, ENUM_32, ENUM_4, ENUM_41, ENUM_5, ENUM_6, ENUM_7, ENUM_81, ENUM_101, ENUM_102, ENUM_11, \
...
U16	device_pprog;
char device_valid;
static	U8	stat_enable;				// status enable
...
//	char	pm = FALSE;				// M flag, MAIN (set if "-M" found in args)
	char	ph = FALSE;				// H flag, (set if "-h" found in args)
...
// ====== PARSING SWITCH ==========================================================================================================================================
			...
			switch(cmd_id){														// dispatch command
				...
				case info:														// info + version
					...
					switch(params[0]){
					case 10:
						send_stat(STAT_ENAB, 0, (char*)NULL);
						cata_time(0xff);						// force timer reset
						send_stat(MAIN, 'R', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(MAIN, 'F', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(SUB,  'F', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(MAIN, 'O', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(SUB,  'O', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(MAIN, 'S', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(SUB,  'S', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(0,    'I', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(MAIN, 'T', srbuf);
						cata_time(0xff);						// force timer reset
						send_stat(SUB,  'T', srbuf);
						break;

					case 11:
						send_stat(STAT_DISAB, 0, (char*)NULL);
						putsQ("#-STAT_DISAB$");
						break;
...
/*						key_hold = c;
				...
				case set_bank:
					...
					if(j){
						hh = nvaddr(0, IDLE_BANK) / BANK_LEN;
						sprintf(obuf,"#-OK %d$", hh);
						putsQ(obuf);
					}else{
						putOK(0);			// error
					}
...
//																		frq  ofs  dplx/pwr/skp  pl   sq            vol           xit           rit           bid           namestr
				...
				case sav_mem:
					...
					// 1234567890123456789012345678901234567890123456789012345678901234567890
					//          1         2         3         4         5         6         7
/*					params[0] = 0;								// band#		// SAV <mid> <mchr>
					get_Dargs(1, nargs, args, params);							// parse param numerics into params[] array
					params[1] = mem2ordinal(args[2][0]);						// convert mem chr to mem#
					if(--params[0] > 5) params[1] = NUM_MEMS;
					if(params[1] < NUM_MEMS){
//b, params[1], c, &ii, &hh, &mem_buf8[0], &fa, &mem_buf8[2], &mem_buf8[3], &mem_buf8[4], &mem_buf8[5], &mem_buf8[6], gp_buf);
						s = get_nameptr(focus);									// get pointer to namestr & calc checksum
						s += 2;
						ii = get_freqb;
//																          frq  ofs  dplx/pwr/skp  pl   sq            vol           xit           rit           bid           namestr
						sprintf(obuf,"\"%d %d %d %f %d %d %d %d %d %s\"", ii,   hh,  mem_buf8[0], fa,  mem_buf8[2],  mem_buf8[3],  mem_buf8[4],  mem_buf8[5], params[0], s);
						for(i=0, c=0, t=obuf+1; *t && (*t != '"'); t++){
							c += *t;
						}
						sprintf(gpbuf,"sav %d %d %d ", params[0]+1, params[1], c);
						putsQ(gpbuf);
						putsQ(obuf);
					}else{
						putOK(0);			// error
...
void cntxt_stat(U8 focus){
	send_stat(focus, 'F', srbuf);
	send_stat(focus, 'S', srbuf);
	send_stat(focus, 'O', srbuf);
	send_stat(focus, 'T', srbuf);
	if(focus == MAIN){
		send_stat(focus, 'I', srbuf);
	}
	return;
}
///////////////////////////////////////////////////////////////////////////////
...
// MAIN status:
//	#S<dplx><bflagH><bflagL><vol><squ><tonecode><rit><xit><mem#><xmode><checkH><checkL>$<\n>
...
//	#o<offs_hz><checkH><checkL>$<\n>
// Base Status:
//	#I<installed><nvsn-xxxx><checkH><checkL>$<\n>
// MemStr:
//	#T<memstr><checkH><checkL>$<\n>
//	#t<memstr><checkH><checkL>$<\n>
// IPL status
//	#Z<IPL><checkH><checkL>$<\n>
//-----------------------------------------------------------------------------
void send_stat(U8 focus, U8 type, char* sptr){
	...
	U8		k;
	U8		m;
	U8		g = 0;					// change trap flag
	U8		ptt_lcl;
	static	U8	srfm_mem;			// change regs
	static	U8	srfs_mem;
	static	U8	cos_mem;
	static	U8	stat_mem;

	if(focus == STAT_IPL){											// IPL init of the statics
		srfm_mem = 0xff;
		srfs_mem = 0xff;
		cos_mem = 0xff;
		stat_mem = 0xff;
	...
	if((!stat_enable) && (type != 'Z')) return;
	if(focus & SEND_STAT_PTT) ptt_lcl = VMODE_ISTX;
	else ptt_lcl = 0;
	focus &= ~SEND_STAT_PTT;
	switch(type){
	default:
	...
	case 'R':
		if(!catz_time(0)){											// use the activity timer to slow down the SRF rate
			if(!cata_time(0)){										// check activity timer
				g = 1;												// flag activity timeout & reset
				cata_time(1);
			}
			i = get_srf(MAIN);										// gather status results
			j = get_srf(SUB);
			k = get_cos();
			m = (get_changestat() & 0x03) << 4;						// main/sub change status, masked & shifted to hi-nyb
			if(i != srfm_mem){
				g = 1;												// if a change, set trap flag
				srfm_mem = i;										// update mem reg
			}
			if(j != srfs_mem){
				g = 1;
				srfs_mem = j;
			...
			if(g){
				...
				*lptr++ = i | 0x40;
				*lptr++ = j | 0x40;
				*lptr++ = k | m | 0x40;								// combine COS/PTT/and change status => [01ms 0pMS] ms = main/sub change status
																	// p = PTT status, MS = main/sub COS status
				ii = scheck(sptr+1, 4);								// split checksum into 2, 6bit values
				*lptr++ = ((ii >> 6) & 0x3f) | 0x40;
				*lptr++ = (ii & 0x3f) | 0x40;
				*lptr++ = '$';										// close response
				*lptr = '\0';
	...
	case 's':
		...
		*lptr++ = '#';												// open response
		...
		//<flags>
		*lptr++ = (read_dplx(focus) & 0x3f) | 0x40;
		i = get_bflag(focus, 0, 0);
		*lptr++ = (i >> 4) | 0x40;
		*lptr++ = (i & 0x0f) | 0x40;
		*lptr++ = adjust_vol(focus, 0) | 0x40;
		*lptr++ = adjust_squ(focus, 0) | 0x40;
		*lptr++ = (get_ctcss(focus) & 0x3f) | 0x40;
		*lptr++ = get_rit() | 0x40;
		*lptr++ = get_xit() | 0x40;
		*lptr++ = ordinal2mem(get_memnum(focus, 0));				// mem#
		*lptr++ = ordinal2mem(get_callnum(focus, 0));				// call#
		*lptr++ = xmode_stat(focus) | 0x40;							// mem stat
		ii = scheck(sptr+1, 12);									// split checksum into 2, 6bit values
		*lptr++ = ((ii >> 6) & 0x3f) | 0x40;
		*lptr++ = (ii & 0x3f) | 0x40;
		*lptr++ = '$';												// close response
		*lptr = '\0';
	...
	case 'F':
		...
		*lptr++ = '#';												// open response
		...
		jj = get_freq(focus | ptt_lcl) * 1000L;
		sprintf(lptr,"%10d--$",jj);
		ii = scheck(sptr+1, 11);									// split checksum into 2, 6bit values
		*(sptr+12) = ((ii >> 6) & 0x3f) | 0x40;
		*(sptr+13) = (ii & 0x3f) | 0x40;
//		*(sptr+14) = '$';											// close response
//		*(sptr+15) = '\0';
		putsQ(sptr);
		break;
	...
	case 'O':
		...
		*lptr++ = '#';												// open response
		...
		jj = (U32)get_offs(focus) * 1000L;
		sprintf(lptr,"%8d--$",jj);
		ii = scheck(sptr+1, 9);									// split checksum into 2, 6bit values
		*(sptr+10) = ((ii >> 6) & 0x3f) | 0x40;
		*(sptr+11) = (ii & 0x3f) | 0x40;
//		*(sptr+14) = '$';											// close response
//		*(sptr+15) = '\0';
		putsQ(sptr);
		break;
	...
	case 'I':
		...
		*lptr++ = 'I';
		jj = (U32)nvram_sn();
		sprintf(lptr,"%c%c%04d--$",(char)get_present()|0x40,nvbank_nxt(0)|0x40,jj);
		ii = scheck(sptr+1, 7);										// split checksum into 2, 6bit values
		*(sptr+8) = ((ii >> 6) & 0x3f) | 0x40;
		*(sptr+9) = (ii & 0x3f) | 0x40;
		putsQ(sptr);
		break;
	...
	case 'Z':
		...
		sprintf(lptr,"#ZIPL--$");
		ii = scheck(sptr+1, 4);										// split checksum into 2, 6bit values
		*(sptr+5) = (U8)((ii >> 6) & 0x3f) | 0x40;
		*(sptr+6) = (U8)(ii & 0x3f) | 0x40;
		putsQ(sptr);
		break;
	...
	case 'T':
		...
		sprintf(lptr,"#T                --$");
		lptr2 = get_nameptr(focus);
		lptr += 2;
		for(i=0; i<MEM_NAME_LEN; i++){
			if(*lptr2 >= ' '){
				*lptr++ = *lptr2++;
			}else{
				*lptr++ = '`';										// back-tic is null/unknown chr
				*lptr2++;
			}
		}
		if(focus != MAIN){
		...
		ii = scheck(sptr+1, MEM_NAME_LEN+1);						// split checksum into 2, 6bit values
		*(sptr+18) = ((ii >> 6) & 0x3f) | 0x40;
		*(sptr+19) = (ii & 0x3f) | 0x40;
		putsQ(sptr);
		break;
	}
...
///////////////////////////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------
// stat_enab() returns stat_enable reg
//-----------------------------------------------------------------------------
U8 stat_enab(void){
