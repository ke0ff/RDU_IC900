...
#define TXD_BUFF_END 100		// CCMD tx buffs
#define TXD1_BUFF_END 256		// CLI tx1 buff
...
U32 config_uart1(U32 baud){
	...
	UART1_LCRH_R |= UART_LCRH_WLEN_8; // | UART_LCRH_STP2;			// 8 bits, 2 stop
	UART1_IM_R = UART_IM_RXIM;									// enable RX interrupt
	UART1_CTL_R |= UART_CTL_RXE | UART_CTL_TXE; // | UART_CTL_EOT;	// set TE, RE, & EOT
...
//	while((UART0_FR_R & 0x0020) == 0x0020);
	...
	// deep buffer driven TXD system
	// drop data into the buffer
	txd1_buff[txd1_hptr++] = c;						// put chr into circ-buff and update head ptr
	if(txd1_hptr >= TXD1_BUFF_END){					// process head wrap-around
		txd1_hptr = 0;
	}
	while((txd1_tptr == txd1_hptr) && (UART1_IM_R & UART_IM_TXIM)){ // if head == tail, flag exception and wait
//		txd1_stat |= TXD_ERR;						// set overrun error
	}
	if(c == '\n'){
		txd1_buff[txd1_hptr++] = '\r';					// put <CR> into buff
		if(txd1_hptr >= TXD1_BUFF_END){					// process head wrap-around
			txd1_hptr = 0;
		}
		while((txd1_tptr == txd1_hptr) && (UART1_IM_R & UART_IM_TXIM)){ // if head == tail, flag exception and wait
//			txd1_stat |= TXD_ERR;						// set overrun error
	...
	txd1_buff[txd1_hptr] = '\0';					// put <CR> into buff
/*	if(!(UART1_IM_R & UART_IM_TXIM)){
		// kick-start TX ISR
//		UART1_ICR_R = UART_ICR_TXIC;					// pre-clear TXD intr flag
//		UART1_DR_R = c;
		UART1_DR_R = 0;
		UART1_IM_R |= UART_IM_TXIM; 					// ...enable TX ISR
	}*/
#endif
	return (c);
}
...
//-----------------------------------------------------------------------------
// kickuart1 (UART1) kickstarts the TXISR
//-----------------------------------------------------------------------------
void kickuart1(void){
//	char	c;
	...
	if(!(UART1_IM_R & UART_IM_TXIM)){
		// kick-start TX ISR
/*		c = txd1_buff[txd1_tptr++];						// pull from the buffer
		if(txd1_tptr >= TXD1_BUFF_END){
			txd1_tptr = 0;								// wrap buffer ptr
		}
		UART1_DR_R = c;
		UART1_IM_R |= UART_IM_TXIM; 					// ...enable TX ISR*/
		UART1_DR_R = '\0';
...
//-----------------------------------------------------------------------------
// puts1() does puts to UART1.  !! ADDS NEWLINE !!
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// getch01 checks for input @ RX0.
//	returns '\0' if no chr
//	looks for input in rxd1_buff[] which is filled in the UART0 interrupt
//-----------------------------------------------------------------------------
char getch01(void){
...
void rxd1_intr(void){
	...
	jj = UART1_MIS_R;
	if((jj & ~(UART_RIS_RXRIS|UART_RIS_TXRIS))){					// if intrpt is NOT rxd,
		// clear ALL intr flags and abort ISR
		UART1_ICR_R = ~jj;
		//UART_ICR_9BITIC|UART_ICR_OEIC|UART_ICR_BEIC|UART_ICR_PEIC|UART_ICR_FEIC|UART_ICR_RTIC|UART_ICR_TXIC|UART_ICR_CTSMIC;
		return;
	}
	// process TX
	if(jj & UART_RIS_TXRIS){
//		while(!(UART1_FR_R & UART_FR_TXFE)){			// process TXD chrs
			if(txd1_buff[txd1_tptr]){
				c = txd1_buff[txd1_tptr++];				// pull from the buffer
				UART1_DR_R = c;
				if(txd1_tptr >= TXD1_BUFF_END){
					txd1_tptr = 0;						// wrap buffer ptr
			...
			if(txd1_hptr == txd1_tptr){
				UART1_IM_R &= ~UART_IM_TXIM;			// turn off TX ISR
			}
//		}
